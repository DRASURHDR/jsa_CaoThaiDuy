...
if (logoutBtn) {
    logoutBtn.addEventListener('click', () => {
        fetch(`${window.location.origin}/wp-content/themes/nolapnolife/auth.php?action=logout`)
            .then(res => res.json())
            .then(data => {
                if (data.success) {
...
document.addEventListener('DOMContentLoaded', function() {
    function checkLoginStatus(callback) {
        fetch('<?php echo site_url(); ?>/wp-content/themes/nolapnolife/auth.php?action=session')
            .then(res => res.json())
            .then(data => {
